...
В этом посте мы разберёмся, какие типы можно в качестве обёрток.

## Какого рода типы могут быть обёртками?

Если каждое вычислительное выражение должно быть ассоциировано с типом-обёрткой, тогда какого рода типы можно использовать в качестве таких обёрток?
...
Например, как мы видели, можно использовать `Option<T>`, `DbResult<T>` и другие подобные типы, как обёртки.

А что насчёт других обобщённых типов, таких как `List<T>` или `IEnumerable<T>`?
...
Нельзя ли в этом случае как-нибудь искусно трактовать `string` как тип-обёртку над `int`?
Это было бы здорово, правда?

Давайте попробуем.
Мы можем использовать сигнатуры методов `Bind` и `Return`, как отправную точку для нашей реализацию.
...
Вот реализация соответствующего класса-строителя:
...
Я не знаю.
Но я бы постарался избегать такой ситуации, и попробовал бы что-нибудь другое, например, опциональный тип, который согласован во всех ситуациях.
...
* `List.collect` — библиотечная функция, которая делает именно это.
...
Скажем, мы можем заменить `let!` на выражение `for..in..do`:
...
В общем случае он делает то же самое, что и `Bind`, но традиционно используется с типами, хранящими последовательности.
...
Иными словами, тип-обёртка был идентичен незавёрнутому типу.
...
Это функция не из конкретного мира обычных значений, а из абстрактного мира типов, но как и любая другая функция она отображает одни значения в другие.  Однако, её входные значения — это типы (`int` и `string`), и выходные значения — тоже типы (`List<int>` и `List<string>`).
...
Если вы ухватили концепцию функций, которые генерируют один тип из другого (они называются "конструкторами типов"), вам становится очевидно, что "тип-обёртка" — это всего лишь конструктор.
...
Теперь, во всём разобравшись, мы можем считать пример с логгированием обычным идентичным процессом с дополнительной логикой логгирования.
...
Вот сводка основных тем, которые мы затронули:
