...
Но разве на самом деле это всё?
...
Если модель не делает ограничения явными, легко столкнуться с рассогласованием при передачи значения от службы к службе.
...
Другая проблема в том, что мы «сломали» сравнение строк, а это уже серьёзно.
...
Воспользуемся тем, что в F# есть интерфейсы и создадим общий интерфейс для типов-обёрток, описав несколько стандартных методов.
...
но я предпочитаю короткий однострочный вариант:
