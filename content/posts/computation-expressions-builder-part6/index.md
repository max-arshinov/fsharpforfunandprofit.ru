...
* В начале цикла `while` есть проверка, которая вычисляется перед каждой итерацией, чтобы определить, надо ли выполнять тело цикла.
  Если результат вычисления равен `false`, «выходим» из цикла.
  В вычислительных выражениях проверка известна как **охранное выражение**.
