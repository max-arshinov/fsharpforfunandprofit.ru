...
Но сейчас у нас появилась другая проблема — мы больше не можем вернуть `None` в явном виде в конструкции `return! None`, вместо этого мы должны вернуть тип `Maybe`.
...
Этот код использует `None` типа `Option<obj>` (предположительно из-за неявной упаковки), однако, компилятор знает, что тип может быть более общим.
...
Альтернативный подход заключается в том, чтобы использовать в качестве внутреннего типа `Maybe<'a>`. Это сделает код более согласованным, но, в то же время, более сложным для понимания.
...
* *Является ли код дорогим с точки зрения ресурсов, может ли результат отличаться при последующих запусках (т.е. является код недетерминированным), или содержит существенные побочные эффекты?*
...
* F# не пытается быть чистым функциональным языком, так что почти весь код на F# попадёт в одну из этих двух категорий.
  Но, *если вам нужен код, в котором гарантированно нет побочных эффектов или вы хотите быть уверены, что дорогой с точки зрения ресурсов код выполняется не больше одного раза*, используйте третий — ленивый — вариант.
