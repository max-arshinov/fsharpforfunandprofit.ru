...
Эта страница предлагает несколько способов (некоторые способы), как учить F# еффективно.
...
Если вы как большинство людей, ваш стандартный подход к изучению нового языка программирования заключается в поиске способов имплементации идей, с которыми вы уже знакомы.
...
Изучая F#, **вам не стоит использовать уже известные вам идеи императивного программирования**. 

>In a pure functional language there are no variables, there are no loops, and there are no objects!

В чистых функциональных языках отсутствуют переменные, нет циклов, и даже нет объектов!

>Yes, F# is a hybrid language and does support these concepts. 

Да, F# является гибридным языком и поддерживает эти идеи. 
...
Для того чтобы получить максимальную пользу от F# и свободно владеть функциональным программированием в целом, критически важно мыслить функционально (в функциональном стиле), а не императивно (в императивном).
...
именно работает F#, особенно ключевые идеи, включая функции и систему типов. 
...
Так что, пожалуйста, прочтите и перечитайте серию ["думать функционально (в функциональном стиле)"](/series/thinking-functionally.html) and ["понимание F# типов"](/series/understanding-fsharp-types.html), поиграйтесь с примерами, и освойтесь с идеями прежде чем приступать к серьёзному (настоящему) кодингу (программированию). 
...
> ## Dos and Don'ts ##
## Что можно и нельзя (Делайте и не делайте) ##

> Here is a list of dos and don'ts that will encourage you to think functionally. 

Вот список того что можно делать и нельзя, который воодушевит вас думать функционально (в функциональном стиле)
...
Поначалу это будет сложно, но как и в изучении иностранного языка, вам необходимо погрузиться (углубиться) и заставить себя говорят как местные.

>* Don't use the `mutable` keyword **at all** as a beginner. 

* Не используйте ключевое слово `mutable` **совсем**, как новичок (на начальном этапе). 

>Coding complex functions without the crutch of mutable state will really force you to understand the functional paradigm.

Кодирование сложны фунций без "костылей" изменяемого (мутабельного) состояния действительно заставит вас понять функциональную парадигму.
...
Используйте сопоставление с шаблоном для булевых значений и рекурсивный просмотр списков.

>* Don't use "dot notation". 

* Не используйте точечную запись.
...
Это может показаться лишней работой, но этот метод работы (подход) естественнен при использовании труб (пайпов) и функций высшего порядка, таких как `List.map`.
...
См. [этот пост для подробностей](/posts/type-extensions/#downsides-of-methods).

>* As a corollary, don't create classes.

* Как следствие, не используйте классы.
...
И конечно, нет инструмента для отладки компилятор и пошагового выполнения его обработки.
...
Как только вы поймёте как их использовать, вы будете на правильном пути к понимаю функций высшего порядка в принципе (вообщем).
...
Рекурсия может быть подвержена ошибкам, и может быть сложно удостовериться что она является правильной хвостовой рекурсией.
...
* Используйте трубу (пайп) (`|>`) и композицию (`>>`) настолько часто как можете.
...
Если вы слепо пишите большое количество кода, а затем пытаетесь его скомпиллировать, вы можете оказаться в куче болезенные и трудно отлаживаемых ошибках компиляции.
