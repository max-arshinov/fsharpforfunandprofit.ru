...
Чтобы создать файл, который будет компилироваться, как часть проекта, вместо файлов сценария используйте файлы с расширенем `.fs` <!-- в оригинале: "вместо файлов используйте расширения `.fs`, что не совсем точно -->. Файлы `.fsx` компилироваться не будут.
