...
Если вы не хотите ничего устанавливать, вы можете попробовать [эмулятор .NET](https://dotnetfiddle.net/), сайт, который предоставляет интерактивную среду, где вы можете исследовать F# в вашем браузере. <!-- "браузер", мне кажется, уже прочно осел в русском языке. Предложить в словарь. -->
...
Я бы рекомендовал подход с файлом сценария для всего, что больше одной или двух строк кода. <!--  Обсудить в словаре: script это сценарий или скрипт? -->
...
После того, как вы скачаете и установите F#, вы можете также установить [F# power pack](http://fsharppowerpack.codeplex.com), который предоставляет некоторое количество прекрасных добавлений, на некоторые из которых ссылается этот сайт.
...
Далее, убедитесь в том, что интерактивное окно F# активно (обычно через `View > Other Windows > F# Interactive`).
...
Использование файла сценария это простейший путь <!-- способ --> экспериментировать с F#; просто вбивайте любой код в окно сценария и запускайте его, чтобы увидеть результаты <!-- дословно: "вывод", "то, что этот код выведет" --> в интерактивном окне ниже.
...
* Щёлкните правой кнопкой, чтобы вызывать контексное меню и выбрать "Execute In Interactive".
  <!-- Проверил в своей студии 2019, пункт меню называется именно так. Кажется, что его переименовали. Русской студии у меня нет, в Google не нашёл, как это переведено там. Если у кого есть, посмотрите, напишите в комментарии. !-->
...
> #### Предупреждение по поводу Resharper <!-- Оригинальный заголовок короткий, и по стилю соответствует чему-то вроде "Внимание! Resharper". -->
...
Но в этом случае вы должны всегда завершать блок кода парой символов "точка с запятой".
...
* В Emacs есть <!-- разработан --> режим F#, который добавляет подсветку синтаксиса F#.
...
Если при компиляции вашего кода возникли проблемы, страница ["Исправление проблем F#"](/troubleshooting-fsharp/) может оказаться полезной.
...
Чтобы создать файл, который будет компилироваться, как часть проекта, вместо файлов сценария используйте файлы с расширенем `.fs` <!-- в оригинале: "вместо файлов используйте расширения `.fs`, что не совсем корректно. -->. Файлы `.fsx` компилироваться не будут.
...
* Вы можете изменять порядок файлов по правой кнопке мыши, выполняя "Move Up" или "Move Down".
  Точно также, создавая новый файл, вы можете выбрать "Add Above" или "Add Below" относительно существующего файла. <!-- Как переводить пункты меню? У меня нет русской студии! -->
...
Вы также можете использовать F#, как язык сценариев, вместо того, чтобы компилировать код в EXE <!-- в исполняемый файл -->.
...
Чтобы увидеть, как вы можете сделать это сами, я написал для вас примерчик сценария, который скачивает <!-- сохраняет --> веб-страницу в локальный файл.
...
Чтобы запустить этот сценарий, откройте командную строку <!-- в оригинале: командное окно --> в том же каталоге, и наберите:
...
Вы можете поэкспериментировать с этим сценарием или написать несколько своих. <!-- Дословно: "Пока вы играете с кодом на этом сайте, вы можете в то же время захотеть поэкспериментировать, создавая свои сценарии." -->